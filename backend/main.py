...
app = FastAPI(title="AI Video Studio H3", version="1.1.0")
...
async def health():
    return {
        ...
        "h3_workflow": workflow_exists(WORKFLOW),
    }
...
async def generate(req: GenerateRequest):
    ...
    if not workflow_exists(WORKFLOW):
        raise HTTPException(503, "Workflow H3 absent. Exporte le workflow ComfyUI en format API vers workflows/h3_t2v.json")
    try:
...
async def job(prompt_id: str):
    try:
        data = await comfy.history(prompt_id)
        entry = data.get(prompt_id)
        if not entry:
            return {"status": "queued_or_running", "prompt_id": prompt_id}
        outputs = comfy.extract_outputs(entry)
        return {"status": "completed", "prompt_id": prompt_id, "outputs": outputs}
    except Exception as e:
